...
- [x] **QUAL-04** `src/components/ui/modal.tsx:59-74` — `DialogContent` nested inside `DialogOverlay` _(30 min)_ — turned out worse than "nested wrong": shadcn's own `DialogContent` already wraps itself in its own `DialogPortal`/`DialogOverlay` internally, so `modal.tsx`'s manual outer wrapping meant every modal rendered two portals and two overlays stacked on top of each other. Removed the outer `DialogPortal`/`DialogOverlay` entirely — `DialogContent` now renders directly inside `Dialog`, the standard shadcn usage pattern. Not smoke-testable via curl (every modal instance sits behind Clerk auth); verified via `pnpm type-check`/`lint` and reading the actual `dialog.tsx` primitive to confirm the double-portal diagnosis before fixing it
- [x] **QUAL-06** `prisma/schema.prisma:34` — `Column.status` is an unconstrained `String` _(30 min / 2 h with migration)_ — the 30-min half was already closed by SEC-07 (`columnStatusSchema.safeParse` on both create and update). Discussed the remaining 2h enum-migration option with the user and deliberately kept `status` as `String`, to leave room for a future user-defined column names feature; the app-layer validation is the enforcement
- [x] **SEC-08** `src/schemas/task.ts:6-9` — unbounded description, untrimmed title _(10 min)_ — added `.trim()` to `title` and `.trim().max(2000)` to `description`, matching `boardSchema`'s pattern
- [x] **PERF-04** `src/stores/loading.ts:17-22` — `isLoading` scans the whole map per render _(1.5 h)_ — added a per-entity+operation `operationCounts` map maintained alongside `loadingMap` in `setIsLoading`; the no-id query path is now an O(1) lookup instead of `Array.from(...).some(startsWith)`. Verified all 16 `setIsLoading` call sites always pass an id before implementing, so no caller's behavior changed — only the "any item of this type loading" query got cheaper
- [x] **PERF-05** `src/utils/performance.ts` — dead code _(5 min)_ — already removed in a prior commit outside this audit's tracked fixes; confirmed via `git log` and a repo-wide grep before ticking
- [x] **FEAT-01** column reordering does not exist despite being marked done _(6–8 h)_ — built end-to-end: `updateColumnPosition` in the DAL (ownership-checked, verifies every id in the new order actually belongs to the board before writing), `updateColumnPositionAction` + `columnPositionSchema`, a `reorderColumns`/`rollbackReorder` pair on the column store (separate snapshot shape from the existing per-item one, so add/update/delete rollback is untouched), a new `use-column-dnd-handlers.ts` hook, and a horizontal `SortableContext` in `ColumnsWrapper`. `ColumnCard` is now itself sortable via a dedicated grip handle in `ColumnHeader` (mirrors the A11Y-01 pattern — the whole card is not the drag target). `DndProvider` now dispatches between task and column handlers based on `active.data.current?.type`, so the existing task DnD code is untouched, not rewritten. Added a lightweight `ColumnDragOverlay` rather than reusing `ColumnCard` in `DragOverlay`, since a second `useSortable` registration under the same id while the real card is still mounted would conflict. Extended `dnd-announcements.ts` so screen readers describe column drags correctly instead of misreporting them as tasks. Known minor cosmetic overlap: the pre-existing task "drop zone" pulse indicator also flashes during column-over-column dragging, since both share `useSortable`'s `isOver`; not fixed, purely cosmetic. Not build- or browser-verified per your instruction — checked with `pnpm type-check`/`lint` plus a full manual trace of the create→persist→rollback path
- [x] **FEAT-02** `Task.dueDate` is stored and rendered but cannot be set _(3–4 h)_ — added `dueDate` to `taskSchema` (`z.iso.date()` or empty string, transformed to `string | null`), a new `"date"` `FormField` type, and a due-date field in `task-form.tsx`. Wired through `createTask`/`updateTask` in the DAL and both server actions, including a proper `dueDateChanged` check in `updateTaskAction`'s "no changes detected" comparison so editing only the date isn't silently ignored. `use-task-form-action.ts`'s optimistic task now carries the real submitted date instead of a hardcoded `null`, and patches `dueDate` into the optimistic edit. No Prisma migration needed — the column already existed, this closed the app-layer gap only
